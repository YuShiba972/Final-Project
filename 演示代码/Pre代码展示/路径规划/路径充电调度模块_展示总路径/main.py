...
if __name__ == "__main__":
    ...
    best_path_points = []
    # 蚁群类初始化
    ant = AntColony(heu_mat, num_iterations=num_iterations, num_ants=8)
    ...
    draw_datas = []
    dis_list = []
    # best_path_location.append(points_list[0].location)
    print('------开始读取绘图数据------')
    ...
    BarGoal_fig.save(Surface=way_picture, filename='Result_Package/移动充电小车游行图.jpg')
    image = Image.open('Result_Package/移动充电小车游行图.jpg')
    image.show()
    # ##########################################################
