...
def ACO_first(points_list, datas, heu_mat, max_battery=10800, consume=5, thred=0.2, v=0.5, eff=30,
              num_iterations=50, alpha=5.0, beta=2.0, rho=0.5, shows='False'):
    ...
    # ant.draw_ant_path(points=list(map(lambda x:x.location,points_list),ant_path=best_path)
...
class AntColony:
    def __init__(self, distances, start_city=0, num_iterations=50,
                 num_ants=25, alpha=5.0, beta=2.0, rho=0.5, Q=1.0):
    ...
    def run(self):
        ...
        best_distance = np.inf  # 将平衡因子初始化为无穷
        best_path = None
        for it in range(self.num_iterations):
            ...
            # self.iteration_best.append(min(list(map(lambda x: x[1], paths))))
            #####################################################################################
            # 信息素挥发更新
            ...
            for path, distance in paths:
                for i in range(len(path) - 1):
                    ...
                    pheromone[path[i + 1], path[i]] += self.Q / distance
    ...
    def plot_iteration_distances(self):
        ...
        plt.plot(x, self.iteration_avgbest, label='每代平均平衡因子')
        plt.plot(x, self.iteration_best, label='每代最优平衡因子')
        plt.legend()
        plt.xlabel('迭代次数')
        plt.ylabel('平衡因子值')
        plt.title('蚁群迭代图')
    ...
    def draw_ant_path(self, points, ant_path, title='Path'):
        """
        绘制蚂蚁迭代过程路径图
        :param points: 各城市坐标点
