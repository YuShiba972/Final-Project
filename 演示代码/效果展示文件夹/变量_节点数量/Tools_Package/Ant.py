...
class AntColony:
    def __init__(self, distances, num_iterations, num_ants,
                 start_city=0, alpha=5.0, beta=2.0, rho=0.5):
        ...
        for it in range(self.num_iterations):
            ...
            for path, distance in paths:
                for i in range(len(path) - 1):
                    pheromone[path[i], path[i + 1]] += distance * 200000
                    pheromone[path[i + 1], path[i]] += distance * 200000
