...
def ACO_first(points_list, datas, heu_mat, max_battery=10800, consume=20, thred=0.2, v=0.5, eff=30,
              num_iterations=50, alpha=5.0, beta=2.0, rho=0.5, Q=1, iteration_fig='False'):
    ant = AntColony(heu_mat, num_iterations=num_iterations, alpha=alpha, beta=beta, rho=rho, Q=Q,
                    num_ants=int(heu_mat.shape[0] * 1.5))
    best_path, best_distance = ant.run()

    if iteration_fig == 'True':
...
class AntColony:
    def __init__(self, distances, start_city=0, num_iterations=50,
                 num_ants=25, alpha=5.0, beta=2.0, rho=0.5, Q=1):
    ...
    def run(self):
        ...
        pheromone = np.multiply(pheromone, num_cities)
        ...
        for it in range(self.num_iterations):
            ...
            self.iteration_best.append(max(list(map(lambda x: x[1], paths))))
            #####################################################################################
            # 信息素挥发更新
            ...
            for path, distance in paths:
                for i in range(len(path) - 1):
                    pheromone[path[i], path[i + 1]] += distance * self.Q
                    pheromone[path[i + 1], path[i]] += distance * self.Q
    ...
    def plot_iteration_distances(self):
        ...
        plt.plot(x, self.iteration_avgbest, label='当代单周期平均效益')
        plt.plot(x, self.iteration_best, label='当代单周期最优效益')
        plt.legend()
        plt.xlabel('迭代次数')
        plt.ylabel('优先级参数')
        plt.title('蚁群迭代图')
