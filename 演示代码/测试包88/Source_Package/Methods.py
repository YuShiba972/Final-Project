...
def charge_first(points_list, datas, max_battery=10800, consume=20, thred=0.2, v=0.5, eff=30):
...
def path_first(points_list, datas, max_battery=10800, consume=20, thred=0.2, v=0.5, eff=30):  # NJNP
    goals_l = points_list[1::]
    origin_energy = list(map(lambda x: x.battery, goals_l))
    # print(origin_energy)
    pl = copy.copy(points_list)
...
def ACO_first(points_list, datas, heu_mat, max_battery=10800, consume=20, thred=0.2, v=0.5, eff=30,
              num_iterations=200, alpha=5.0, beta=2.0, rho=0.5):
    ant = AntColony_plus(heu_mat, num_iterations=num_iterations, alpha=alpha, beta=beta, rho=rho,
...
class AntColony:
    def __init__(self, distances, start_city=0, num_iterations=50,
                 num_ants=25, alpha=5.0, beta=2.0, rho=0.1):
    ...
    def plot_iteration_distances(self, iteration_distances):
        import matplotlib.pyplot as plt
        num_iterations = len(iteration_distances)
        x = list(range(num_iterations))
        y_mean = [sum(iteration_distances[:i + 1]) / (i + 1) for i in range(num_iterations)]
        y_best = [max(iteration_distances[:i + 1]) for i in range(num_iterations)]

        plt.plot(x, y_mean, label='Mean Distance')
        plt.plot(x, y_best, label='Best Distance')
        plt.legend()
        plt.xlabel('Iteration')
        plt.ylabel('Distance')
        plt.title('Iteration Distances')
        plt.show()


class AntColony_plus:
    def __init__(self, distances, start_city=0, num_iterations=50,
                 num_ants=25, alpha=5.0, beta=2.0, rho=0.5, C=1):
        self.distances = distances
        self.start_city = start_city
        self.num_iterations = num_iterations
        self.num_ants = num_ants
    ...
    def plot_iteration_distances(self):
        import matplotlib.pyplot as plt
        num_iterations = len(self.iteration_best)
        x = list(range(num_iterations))
        # y_mean = [sum(self.iteration_best[:i + 1]) / (i + 1) for i in range(num_iterations)]
        # y_best = [max(self.iteration_best[:i + 1]) for i in range(num_iterations)]

        plt.plot(x, self.iteration_avgbest, label='Mean')
        plt.plot(x, self.iteration_best, label='Best')
        plt.legend()
        plt.xlabel('Iteration')
        plt.ylabel('Distance')
        plt.title('Iteration Figure')
        plt.show()
