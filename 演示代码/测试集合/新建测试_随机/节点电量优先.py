...
if __name__ == "__main__":
    ...
    # 横轴变量求值
    va = [5400, 7200, 9000, 10800, 13600]
    ylab1 = []
    ylab2 = []
    ylab3 = []
    ylab4 = []

    for max_battery in va:
        Gain1_l = []
        Gain2_l = []
        Gain3_l = []
        Gain4_l = []
        for i in range(100):
            Gain1, Gain2, Gain3, Gain4 = charge_first(points_list=points_list, max_battery=max_battery)
    ...
    plt.subplot(221)
    plt.plot(va, ylab1, 'o--', alpha=0.5, linewidth=1, label='MMAS-CM')  # 'bo-'表示蓝色实线，数据点实心原点标注
    plt.legend()  # 显示上面的label
    plt.xlabel('电池最大电量')  # x_label
    plt.ylabel('收益比')  # y_label
    plt.ylim(100, 4000)  # 仅设置y轴坐标范围

    plt.subplot(222)
    plt.plot(va, ylab2, '*--', alpha=0.5, linewidth=1, label='MMAS-CM', color='red')  # 'bo-'表示蓝色实线，数据点实心原点标注
    plt.legend()  # 显示上面的label
    plt.xlabel('电池最大电量')  # x_label
    ...
    plt.subplot(223)
    plt.plot(va, ylab3, 'o--', alpha=0.5, linewidth=1, label='MMAS-CM', color='yellow')  # 'bo-'表示蓝色实线，数据点实心原点标注
    plt.legend()  # 显示上面的label
    plt.xlabel('电池最大电量')  # x_label
    ...
    plt.subplot(224)
    plt.plot(va, ylab4, 'o--', alpha=0.5, linewidth=1, label='MMAS-CM', color='gray')  # 'bo-'表示蓝色实线，数据点实心原点标注
    plt.legend()  # 显示上面的label
    plt.xlabel('电池最大电量')  # x_label
