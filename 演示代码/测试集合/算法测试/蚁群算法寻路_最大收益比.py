...
goal_locations = [[16, 10], [4, 9], [10, 11]]  # 目标坐标矩阵

n = len(goal_locations)  # 城市数量
...
x = []  # 存放各城市横坐标
y = []  # 存放各城市纵坐标
for i in range(0, len(goal_locations)):  # 取值
    route = goal_locations[int(Shortest_Route[i])]
    x.append(route[0])
    y.append(route[1])
...
plt.plot(end2Start_x[:-1], end2Start_y[:-1], color='orange')  # 连接终点和起点，形成回路
