...
def charge_first(points_list, datas, max_battery=10800, consume=200, thred=0.2, v=1):
    ...
    while pl:  # 设置终止条件,未访问的节点为空
        ...
        for point in goals_l:
            point.battery = point.battery - (tem_dis / v) * consume
        # 低于阈值的节点判定
        for point in goals_l:
...
def path_first(points_list, datas, max_battery=10800, consume=200, thred=0.2, v=1):  # NJNP
    goals_l = points_list[1::]
    origin_energy = list(map(lambda x: x.battery, goals_l))
    pl = copy.copy(points_list)
...
def ACO_first(points_list, datas, bat_mat, dis_mat, max_battery=10800, consume=200, thred=0.2, v=1):
    alpha = 1
    beta = 2 - alpha
    ...
    def get_heu_mat(bat_mat, dis_mat):
        ...
        # heu_mat = dis_mat
        # heu_mat = np.multiply(bat_mat, alpha) / np.multiply(dis_mat, beta)
        return heu_mat

    heu_mat = get_heu_mat(bat_mat, dis_mat)
    best_path, best_distance = AntColony(dis_mat).run()
...
class AntColony:
    def __init__(self, distances, start_city=0, num_iterations=50,
                 num_ants=9, alpha=1.0, beta=2.0, rho=0.5):
