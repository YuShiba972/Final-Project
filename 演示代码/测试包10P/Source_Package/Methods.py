...
def charge_first(points_list, datas, max_battery=10800, consume=15, thred=0.3, v=1, eff=1):
    ...
    while pl:  # 设置终止条件,未访问的节点为空
        ...
        for point in goals_l:
            point.battery = point.battery - (tem_dis / v) * consume - (max_battery - current.battery) / eff
        # 低于阈值的节点判定
        for point in goals_l:
    ...
    # print('MMAS-CM distance:', sum(dis_l))
    #######################   性能评定  ###################################
...
def path_first(points_list, datas, max_battery=10800, consume=15, thred=0.3, v=1, eff=1):  # NJNP
    goals_l = points_list[1::]
    origin_energy = list(map(lambda x: x.battery, goals_l))
    pl = copy.copy(points_list)
    ...
    while pl:
        ...
        for point in goals_l:
            point.battery = point.battery - (min_dis / v) * consume - (max_battery - current.battery) / eff
            # 低于阈值的节点判定
            for point in goals_l:
                if point.battery < thred * max_battery:
                    point.battery = thred * max_battery
...
def ACO_first(points_list, datas, bat_mat, dis_mat, max_battery=10800, consume=15, thred=0.3, v=1, eff=1):
    alpha = 1
    beta = 2 - alpha

    def get_heu_mat(bat_mat, dis_mat):
        dis_mat[dis_mat == 0] = 9999
        heu_mat = alpha * bat_mat / beta * dis_mat
        return heu_mat

    heu_mat = get_heu_mat(bat_mat, dis_mat)
    best_path, best_distance = AntColony(heu_mat, num_ants=int(bat_mat.shape[0] * 1.5)).run()
...
class AntColony:
    def __init__(self, distances, start_city=0, num_iterations=50,
                 num_ants=25, alpha=5.0, beta=3.0, rho=0.5):
