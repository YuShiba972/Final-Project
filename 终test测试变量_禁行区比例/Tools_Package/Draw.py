...
def plot_result(arg_l, times=10, shows='single'):
    # 节点数量变化
    barrier_num = [arg[0] / 100 for arg in arg_l]
    # 启发式算法参数
    omiga = 1.0
    gama = 1.0
    # 默认模型参数
    ...
    if shows == 'single':
        ...
        # 画布/画布背景色
        plt.figure().set_facecolor(color=(255 / 255, 255 / 255, 255 / 255))

        ###################################################################################
        plt.subplot(311)
        # 柱状图绘画
        plt.bar(barrier_num, [Gain[0] for Gain in y_lab],
                color=(58 / 255, 152 / 255, 185 / 255), width=0.01)
        # 折线图绘画
        plt.plot(barrier_num, [Gain[0] for Gain in y_lab], linestyle='--',
                 color=(255 / 255, 105 / 255, 105 / 255), marker='*', markersize=6, linewidth=1, label='禁行区面积比例')
        # 子图背景色
        ...
        plt.ylim()
        # 显示数值标签
        for x, y in zip(barrier_num, [Gain[0] for Gain in y_lab]):
            plt.text(x, y + 0.1, str(int(y)), ha='center', va='bottom', fontsize=10)
        ###################################################################################
        plt.subplot(312)
        plt.bar(barrier_num, [Gain[1] for Gain in y_lab],
                color=(58 / 255, 152 / 255, 185 / 255), width=0.01)
        plt.plot(barrier_num, [Gain[1] for Gain in y_lab], linestyle='--',
                 color=(255 / 255, 105 / 255, 105 / 255), marker='o', markersize=6, linewidth=1, label='禁行区面积比例')
        ...
        plt.xlim([0, 0.25])
        plt.ylim()
        for x, y in zip(barrier_num, [Gain[1] for Gain in y_lab]):
            plt.text(x, y + 0.1, str(round(y, 2)), ha='center', va='bottom', fontsize=10)

        ###################################################################################
        plt.subplot(313)
        plt.bar(barrier_num, [Gain[3] for Gain in y_lab],
                color=(58 / 255, 152 / 255, 185 / 255), width=0.01)
        plt.plot(barrier_num, [Gain[3] for Gain in y_lab], linestyle='--',
                 color=(255 / 255, 105 / 255, 105 / 255), marker='o', markersize=6, linewidth=1, label='禁行区面积比例')
        plt.gca().set_facecolor(color=(255 / 255, 255 / 255, 255 / 255))  # 设置背景色
        plt.grid(axis='y')
        ...
        plt.ylim()
        for x, y in zip(barrier_num, [Gain[3] for Gain in y_lab]):
            plt.text(x, y + 0.1, str(int(y)), ha='center', va='bottom', fontsize=10)

        # 保存图片
        plt.savefig('禁行区面积比例_充电收益')
        # 展示图片
        plt.show()
