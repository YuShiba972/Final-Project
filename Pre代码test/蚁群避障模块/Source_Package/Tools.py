...
def cycle_run_one(bar_data_file, points, out_file='单点路径列表_tem.txt'):  # 获取单个目标点到所有点的最短距离集合
    ...
    for k, i in enumerate(points_location_list):
        try:
            ...
            break
        aco = ACO(map_data=map_data, start=points_location_list[0], end=j,
                  max_iter=50, ant_num=300, pher_imp=1, dis_imp=5, evaporate=0.1, pher_init=8)
        aco.run()
        # aco.plot_iteration_path()
        best_path_l.append(aco.way_data_best)  # 存储最优路径列表，形式为[(0,0),(1,2)...]
        distance_l.append(aco.way_len_best)  # 存储对应的距离列表，形式为[l1,l2...]
    # print(best_path_l, distance_l)
    with open(out_file, 'w') as f:
        f.write(str(best_path_l))
    return best_path_l, distance_l
...
def plot_OneToAll(Bg_fig, Surface, input_file, out_file='单点对单点路径展示图.jpg', shows='False'):
    route_color = [[255, 0, 0], [0, 255, 0], [0, 0, 255], [0, 255, 255], [255, 0, 255],
                   [255, 255, 0], [0, 47, 167], [47, 0, 167], [167, 47, 0], [160, 255, 255],
                   [100, 100, 255], [255, 100, 255], [47, 100, 167], [167, 47, 100], [40, 255, 255], ]
    with open(input_file) as f:
...
class Point:
    ...
    def points_ite():
        ...
        B = Point(name='B', location=[1, 18])
        C = Point(name='C', location=[19, 2])
        D = Point(name='D', location=[2, 16])
        E = Point(name='E', location=[17, 5])
        F = Point(name='F', location=[5, 19])
        points_list = [A, B, C, D, E, F]
        return points_list
...
class ACO:
    def __init__(self, map_data, start, end, max_iter=50, ant_num=400, pher_imp=1.0, dis_imp=5.0, evaporate=0.1,
                 pher_init=8) -> None:
        '''
            Params:
            --------
    ...
    def run(self):
        ...
        for i in range(self.max_iter):
            ...
            # self.generation_best.append(way_len_best))
            ...
            if min(way_lenght_list) < self.current_best[-1]:
                self.current_best.append(min(way_lenght_list))
            else:
                self.current_best.append(self.current_best[-1])
        self.current_best.pop(0)
...
def ACO_Generate(bar_data_file, points, filename):
    ...
    for point in points_location_list:
        print('点{}的路径文件开始生成'.format(point.name))
        tem = points_location_list.copy()  # 新分配内存区域
        tem.remove(point)
        for goal_point in tem:
            aco = ACO(map_data=npdata, start=point, end=goal_point, max_iter=50, ant_num=400)
            aco.run()
...
def cycle_run_ite(bar_data_file, points, out_file='Result_Package/蚁群避障迭代图.jpg'):  # 获取单个目标点到所有点的最短距离集合
    points_location_list = list(map(lambda x: x.location, points))
    ...
    for k, i in enumerate(points_location_list):
        try:
            ...
            break
        aco = ACO(map_data=map_data, start=points_location_list[0], end=j,
                  max_iter=100, ant_num=200, pher_imp=1, dis_imp=5, evaporate=0.1, pher_init=8)
        aco.run()
        draw_datas.append((aco.generation_aver, aco.current_best))
        # draw_datas.append((aco.generation_aver, aco.generation_best))

    x = list(range(len(draw_datas[0][0])))
    import matplotlib.pyplot as plt
    plt.rcParams['font.family'] = ['SimHei']
    plt.rcParams['axes.unicode_minus'] = False  # 负号正常显示

    plt.subplot(221)
    # plt.plot(x, draw_datas[0][0], label='每代平均路径长度')
    plt.plot(x, draw_datas[0][1], label='最优路径长度')
    plt.legend()
    plt.xlabel('蚂蚁迭代次数')
    plt.ylabel('避障路径长度')
    plt.title('蚁群算法迭代图(基站点出发)')

    plt.subplot(222)
    # plt.plot(x, draw_datas[1][0], label='每代平均路径长度')
    plt.plot(x, draw_datas[1][1], label='最优路径长度')
    plt.legend()
    plt.xlabel('蚂蚁迭代次数')
    plt.ylabel('避障路径长度')
    plt.title('蚁群算法迭代图(基站点出发)')
    ...
    plt.subplot(223)
    # plt.plot(x, draw_datas[2][0], label='每代平均路径长度')
    plt.plot(x, draw_datas[2][1], label='最优路径长度')
    plt.legend()
    plt.xlabel('蚂蚁迭代次数')
    plt.ylabel('避障路径长度')
    plt.title('蚁群算法迭代图(基站点出发)')
    ...
    plt.subplot(224)
    # plt.plot(x, draw_datas[4][0], label='每代平均路径长度')
    plt.plot(x, draw_datas[4][1], label='最优路径长度')
    plt.legend()
    plt.xlabel('蚂蚁迭代数')
    plt.ylabel('路径长度')
    plt.title('蚁群算法迭代图(基站点出发)')
