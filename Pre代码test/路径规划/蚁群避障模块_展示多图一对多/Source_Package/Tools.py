...
def ACO_Generate(bar_data_file, points, filename):
    ...
    for point in points_location_list:
        start = time.time()
        print('----点{}的路径文件开始生成----'.format(point))
        tem = points_location_list.copy()  # 新分配内存区域
        tem.remove(point)
        for goal_point in tem:
            aco = ACO(map_data=npdata, start=point, end=goal_point, max_iter=50, ant_num=600)
            aco.run()
            datas.append([point, goal_point, round(aco.way_len_best, 2), aco.way_data_best])
        end = time.time()
        print('----点{}生成的路径内容为:{}----'.format(point, datas[-1]))
        print('----点{}的路径文件的生成时间为:{}----'.format(point, round((end - start), 2)))
    # for data in datas:
    #     print(data)
...
def read(Bg_fig, Surface, po_index, input_file='Result_Package/节点避障距离文件.txt', shows='False'):
    route_color = [[255, 0, 0], [0, 255, 0], [0, 0, 255], [0, 255, 255], [255, 0, 255],
                   [255, 255, 0], [0, 47, 167], [47, 0, 167], [167, 47, 0], [160, 255, 255],
                   [100, 100, 255], [255, 100, 255], [47, 100, 167], [167, 47, 100], [40, 255, 255]]
    ...
    if shows == 'True':
        from PIL import Image
        Image.open('Result_Package/单点对多点路径展示图{}.jpg'.format(po_index)).show()
