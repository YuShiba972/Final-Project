...
class Point:
    ...
    def charge_and_get_bat_mat(points_list, max_battery=10800, thred=0.2):
        ...
        print('移动充电小车的出发点为网络基站为{}，坐标为{}'.format(points_list[0].name, points_list[0].location))
        print('------随机生成待充电传感器节点初始电量------')
        for i in range(len(tem_energy_list)):
            print('本次为传感器节点{}，坐标{}，随机生成的初始电量为{} J：'.format(points_list[i+1].name,points_list[i+1].location,tem_energy_list[i]))
        # 能量数据注入点类
    ...
    def points15_bar10():
        ...
        points_list = [A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P]
...
def draw_map(points, bar_data_file, filename='Result_Package/测试节点分布图.jpg', shows='False', save='True'):
    ...
    # shangetu_bar_goalx = result_figure.goal_point(location_l[5::], Surface=shangetu_bar_goal,color=[47,255,167])  # 画目标点
    if save == 'True':
        result_figure.save(Surface=shangetu_bar_goal, filename=filename)
    if shows == 'True':
