...
def threshold(points_list, dis_datas, dis_mat, va_list, times, out_file, va_name='传感器节点休眠门限'):
    # 启发式算法参数
    omiga = 1.0
    gama = 1.0
    ...
    for va in va_list:
        print('----执行门限{}变量测试----'.format(va))
        tem_C_Gain_l, tem_P_Gain_l, tem_ACO_Gain_l = [], [], []
        for i in range(times):
            active_points_list, bat_mat = Point.charge_and_get_bat_mat(points_list, thred=va)
            ...
            tem_ACO_Gain_l.append(ACO_first(points_list=active_points_list3, datas=dis_datas, thred=va,
                                            ...
                                            num_iterations=num_iterations, alpha=alpha, beta=beta, rho=rho))
    ...
    plt.subplot(133)
    opacity, bar_width, index = 0.8, 0.25 * va_list[0], va_list
    plt.bar([i for i in index], [Gain[3] for Gain in C_ylabs], bar_width, alpha=opacity, color='b', label='AS-CM')
    plt.bar([i + bar_width for i in index], [Gain[3] for Gain in P_ylabs], bar_width, alpha=opacity, color='g',
            label='NJNP')
    plt.bar([i + bar_width * 2 for i in index], [Gain[3] for Gain in ACO_ylabs], bar_width, alpha=opacity, color='r',
            label='AS-CNP')
    plt.legend()
    plt.xticks(va_list)
    plt.xlabel(va_name)
...
def max_battery(points_list, dis_datas, dis_mat, va_list, times, outfile, va_name='传感器节点最大电池容量'):
    # 启发式算法参数
    omiga = 1.0
    gama = 1.0
    ...
    C_ylabs, P_ylabs, ACO_ylabs = [], [], []
    for va in va_list:
        print('----执行电量{}变量测试----'.format(va))
        tem_C_Gain_l, tem_P_Gain_l, tem_ACO_Gain_l = [], [], []
        for i in range(times):
            active_points_list, bat_mat = Point.charge_and_get_bat_mat(points_list=points_list, max_battery=va)
            active_points_list1 = copy.deepcopy(active_points_list)
            active_points_list2 = copy.deepcopy(active_points_list)
            active_points_list3 = copy.deepcopy(active_points_list)
    ...
    # 子图3
    plt.subplot(133)
    opacity, bar_width, index = 0.8, 0.25 * 1000, va_list
    plt.bar([i for i in index], [Gain[3] for Gain in C_ylabs], bar_width, alpha=opacity, color='b', label='AS-CM')
    plt.bar([i + bar_width for i in index], [Gain[3] for Gain in P_ylabs], bar_width, alpha=opacity, color='g',
            label='NJNP')
    plt.bar([i + bar_width * 2 for i in index], [Gain[3] for Gain in ACO_ylabs], bar_width, alpha=opacity, color='r',
...
def consume(points_list, dis_datas, dis_mat, va_list, times, outfile, va_name='传感器节点工作功率'):
    # 启发式算法参数
    omiga = 1.0
    gama = 1.0
...
def threshold_ite(points_list, dis_mat, va_list, out_file):
    # 启发式算法参数
    omiga = 1.0
    gama = 1.0
    ...
    for va in va_list:
        ...
        ant.run()
        draw_list.append((va, list(range(len(ant.iteration_best))), ant.iteration_avgbest, ant.iteration_best))
    ...
    plt.subplot(221)
    plt.plot(draw_list[0][1], draw_list[0][2], label='每代平均平衡因子')
    plt.plot(draw_list[0][1], draw_list[0][3], label='最优平衡因子')
    plt.legend()
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器休眠门限值为({})的蚁群迭代图'.format(draw_list[0][0]))

    plt.subplot(222)
    plt.plot(draw_list[1][1], draw_list[1][2], label='每代平均平衡因子')
    plt.plot(draw_list[1][1], draw_list[1][3], label='最优平衡因子')
    plt.legend()
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器休眠门限值为({})的蚁群迭代图'.format(draw_list[1][0]))

    plt.subplot(223)
    plt.plot(draw_list[2][1], draw_list[2][2], label='每代平均平衡因子')
    plt.plot(draw_list[2][1], draw_list[2][3], label='最优平衡因子')
    plt.legend()
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器休眠门限值为({})的蚁群迭代图'.format(draw_list[2][0]))

    plt.subplot(224)
    plt.plot(draw_list[3][1], draw_list[3][2], label='每代平均平衡因子')
    plt.plot(draw_list[3][1], draw_list[3][3], label='最优平衡因子')
    plt.legend()
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器休眠门限值为({})的蚁群迭代图'.format(draw_list[3][0]))

    plt.savefig(out_file)
    plt.show()


def max_batteries_ite(points_list, dis_mat, va_list, out_file):
    # 启发式算法参数
    omiga = 1.0
    gama = 1.0
    ...
    draw_list = []
    for va in va_list[0:4]:
        print('----执行电量({})变量测试----'.format(va))
        active_points_list, bat_mat = Point.charge_and_get_bat_mat(points_list, max_battery=va)
        heu_mat = gama * dis_mat * 1000 + omiga * bat_mat
        ant = AntColony(heu_mat, num_iterations=num_iterations, alpha=alpha, beta=beta, rho=rho,
                        num_ants=int(heu_mat.shape[0] * 1.5))
        ant.run()
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器最大电量为({})的蚁群迭代图'.format(draw_list[0][0]))
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器最大电量为({})的蚁群迭代图'.format(draw_list[1][0]))
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器最大电量为({})的蚁群迭代图'.format(draw_list[2][0]))
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器最大电量为({})的蚁群迭代图'.format(draw_list[3][0]))

    plt.savefig(out_file)
    plt.show()


def consume_ite(points_list, dis_mat, va_list, out_file):
    # 启发式算法参数
    omiga = 1.0
    gama = 1.0
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器节点工作功率({})的蚁群迭代图'.format(draw_list[0][0]))
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器节点工作功率({})的蚁群迭代图'.format(draw_list[1][0]))
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器节点工作功率({})的蚁群迭代图'.format(draw_list[2][0]))
    ...
    plt.xlabel('迭代次数')
    plt.ylabel('平衡因子')
    plt.title('传感器节点工作功率({})的蚁群迭代图'.format(draw_list[3][0]))
