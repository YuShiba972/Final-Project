...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=3) as pool:
        result1 = pool.apply(threshold,
                             (points_list, dis_datas, dis_mat, threds, times, 'Result_Package/门限变量'))
        result2 = pool.apply(max_battery,
                             (points_list, dis_datas, dis_mat, max_batteries, times, 'Result_Package/电量变量'))
        result3 = pool.apply(consume,
                             (points_list, dis_datas, dis_mat, consumes, times, 'Result_Package/消耗变量'))
